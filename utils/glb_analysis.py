...
Master Script for analysis of 129Xenon ventilation MR images using
    1) 99th percentile normalized Generalized linear binning as introduced in:
        He et al., Academic Radiology, 2020
    2) Mean normalized Generalized linear binning as introduced in:
        Hussain et al., ISMRM Abstract, 2025 (Median-anchored)
...
#%%99th percentile normalized generalized linear binning analysis
def percentile_glb(img_msk_prtn, glb_dirs, prcntl_dict, analysis_corr_subjectid):
    """Master function for GLB (percentile and mean) analysis"""
    ...
    if isinstance(glb_dirs, list):
        dirs_n_analysis = [glb_dirs[0], 'glb-percentile']
    else:
        dirs_n_analysis = [glb_dirs, 'glb-percentile']
    main_glb_percentile_func(mskd_norm_img_prtn, dirs_n_analysis, prcntl_dict,
                            analysis_corr_subjectid)

def main_glb_percentile_func(msked_image_prtn, dirs_e_analysis, thresh_fit_params,
                                analyses_correc_subjectid):
    """Master function for percentile glb"""
    msked_image, prtn = msked_image_prtn
    img_1d = msked_image[msked_image != 0]
    thresholds = thresh_fit_params.get('thresholds', {})
    fit_params = thresh_fit_params.get('fits', {})
    # #Plot the single subject histogram with 99th percentile overlay
    percentile_val, percentile_1d_arr = percentile_histograms(img_1d, dirs_e_analysis[0],
                                                            analyses_correc_subjectid[2])
    # # #Make any values above 99th percentile = 1 (max)
    img_percentile_ = np.array(msked_image/percentile_val)
    img_percentile_[img_percentile_ > 1] = 1
    img_percentile_normd = (img_percentile_ - np.min(img_percentile_)) / (
                            np.max(img_percentile_) - np.min(img_percentile_))
    img_percentile_e_prtn = [img_percentile_normd, prtn]
    # ##Overlaying healthy-cohort-fit on single subjects' histogram (99th percentile)
    hist_with_bins(percentile_1d_arr, thresholds, fit_params, dirs_e_analysis,
                   analyses_correc_subjectid)
    binning_cmap_image(img_percentile_e_prtn, thresholds, dirs_e_analysis,
                       analyses_correc_subjectid)

def percentile_histograms(img_1d_array, prcntl_dir=None, data_type=None, prcntl_val=99):
    """
    Plots histograms from input 1D array: 1) all data 2) post 99th percentile.
    The two (percentile) histograms are overlaid on top of each other.
    Parameters:
    img_1d_array: A 1D NumPy array of data.
    prcntl_dir: (Optional) Directory to save histograms.
    data_type: (Optional) String to specifies data type in array and to name output files.
    prcntl_val = (Optional) to control what percentile to take (Default=99th).
    Returns:
    percentile_val: The 99th percentile value of the input array.
    oned_percentile_arr: The 99th percentile 1d array normalized b/w 0 - 1.
    """
    # Plot Non_corr histogram
    plt.hist(img_1d_array, bins=100, histtype='bar', density=True, color='y')
    # Get the 99th percentile value of 1d image
    percentile_val = np.percentile(img_1d_array, prcntl_val, axis =0)
    # Make anything above the 99th percentile =  99th percentile
    img_1d_array[img_1d_array > percentile_val] = percentile_val
    # Plot histogram with 99th percentile limit
    plt.hist(img_1d_array, bins=100, histtype='bar', density=True, color='c')
    #Normalize the 1D data to 1
    oned_percentile_arr = (img_1d_array - np.min(img_1d_array)) / (
                            np.max(img_1d_array) - np.min(img_1d_array))
    # Save overlaid histograms if path provided
    if prcntl_dir is not None:
        filename = os.path.join(prcntl_dir, f"{data_type}_percentile_hist_overlay.png")
        print(f"Histogram saved as: {filename}")
        plt.savefig(filename, dpi=300, bbox_inches='tight')
    plt.show()
    return percentile_val, oned_percentile_arr

#%%Mean normalized generalized linear binning analysis
def mean_glb(img_msk_prtn, glb_dirs, mean_dict, analysis_corr_subjectid):
    """Master function for GLB (percentile and mean) analysis"""
    mskd_img = img_msk_prtn[0] * img_msk_prtn[1]
    mskd_img /= np.max(mskd_img)
    mskd_norm_img_prtn = [mskd_img, img_msk_prtn[2]]
    if isinstance(glb_dirs, list):
        dirs_n_analysis = [glb_dirs[1], 'glb-mean']
    else:
        dirs_n_analysis = [glb_dirs, 'glb-mean']
    # # ##Calculate generalized linear binning VDP/HVP - mean
    main_glb_mean_func(mskd_norm_img_prtn, dirs_n_analysis, mean_dict,
                            analysis_corr_subjectid)

def main_glb_mean_func(msked_image_prtn, dirs_e_analysis, thresh_fit_params,
                         analyses_correc_subjectid):
    ...
    hist_with_bins(mean_1d_arr, thresholds, fit_params, dirs_e_analysis,
                   analyses_correc_subjectid)
...
#%%Common functions
def hist_with_bins(image_1d,thresholds,fit_params, dirs_e_analysis=None,
    ...
    x_data = np.linspace(min(image_1d), max(image_1d), 100)
    # Create colored threshold bins
    _, ax = plt.subplots()
    ax.axvspan(min(image_1d), thresholds[data_type][0], facecolor='#ff0000', alpha=1)
    ax.axvspan(thresholds[data_type][0],thresholds[data_type][1], facecolor='#ffb600', alpha=1)
    ax.axvspan(thresholds[data_type][1],thresholds[data_type][2], facecolor='#66b366', alpha=1)
    ax.axvspan(thresholds[data_type][2],thresholds[data_type][3], facecolor='#00ff00', alpha=1)
    ax.axvspan(thresholds[data_type][3],thresholds[data_type][4], facecolor='#0091b5', alpha=1)
    if thresholds[data_type][4] < max(image_1d):
        ax.axvspan(thresholds[data_type][4], max(image_1d), facecolor='#0000ff', alpha=1)
    # Plot the fit curve
    skew_fit = skewnorm.pdf(x_data, fit_params[data_type][0], fit_params[data_type][1],
                            fit_params[data_type][2])
    plt.plot(x_data, skew_fit, 'k--', linewidth=2.5)
...
def binning_cmap_image(norm_prtn, thresholds, dirs_analysis=None,
    ...
    binned_cmap_img=create_binned_img(norm_prtn[0], thresholds, corr_type)
...
def create_binned_img(norm_mr_image, thresholds, data_type):
    ...
    binned_img[(norm_mr_image > 0) & (norm_mr_image < thresholds[data_type][0])] = 1
    binned_img[(norm_mr_image >= thresholds[data_type][0]) &
                (norm_mr_image < thresholds[data_type][1])] = 2
    binned_img[(norm_mr_image >= thresholds[data_type][1]) &
                (norm_mr_image < thresholds[data_type][2])] = 3
    #notice change of direction for the equal sign
    binned_img[(norm_mr_image >= thresholds[data_type][2]) &
                      (norm_mr_image <= thresholds[data_type][3])] = 4
    binned_img[(norm_mr_image > thresholds[data_type][3]) &
                      (norm_mr_image <= thresholds[data_type][4])] = 5
    binned_img[(norm_mr_image > thresholds[data_type][-1])]= len(thresholds[data_type])+1
    return binned_img
...
def create_binning_montage(binned_cmap_montage, bin_prcnts, prtn_arr=None):
    """Make montage for generalized linear binning maps"""
